...
- `web/` — Next.js app (dashboard + landing). `cd web && npm run dev` → http://localhost:3000
- `docs/SCORING-V2.md` — DealScore v3 scoring spec (implemented in `web/src/lib/scoring/engine.ts`)
...
3. **2 — Bookings & automation: ✅ BUILT** (demo mode). Onboarding flow (tier → payment → parameters → signed agreement), Calendar (30-day grid), Document Vault, Outreach Concierge (cadence timeline, script editor, call logs), Inngest workflows (Tuesday drop, cadence, rescoring — `web/src/inngest/`), email templates, adapters for Resend/Vapi/Lob/Cal.com — **needs: STRIPE/RESEND/CALCOM/INNGEST keys**.
